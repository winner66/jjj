...
Page({
  ...
  getHouseActivityJson: async function (id) {
    let that = this;
    const json = await apis.canaleByBuild({
      data: {
        build_id: id,
      }
    });
  ...
  getqudaoJson:async function(){
    ...
    if (json.count <= 0) {
      ...
      let searchList = [];
      //如果isFromSearch是true从data中取出数据，否则先从原来的数据继续添加  
      that.data.isList ? searchList = data : searchList = that.data.qudao.concat(data)
      that.setData({
        qudao: searchList, //获取数据数组           
        searchLoading: true   //把"上拉加载"的变量设为false，显示 
      });
    }
    //  JSON.parse(str);
  ...
  getHousequdaoJson: async function (id) {
    const that = this;
    const json = await apis.canaleByBuild({
      data: {
       build_id:id
    ...
    //  JSON.parse(str);
    console.log("gethousequdaojson :" + JSON.stringify(this.data.qudao))
  },
  //滚动到底部触发事件
